...
hists = {'fj_prop': hist.Hist("fj",
                              ...
                              hist.Bin("mass", r"fj mass [GeV]", 80, 50, 250),
                              hist.Bin("gmass", r"gen H mass [GeV]", 80, 50, 250),
                          ),
         'h_gen': hist.Hist("h",
                            hist.Bin("gmass", r"gen H mass [GeV]", 80, 50, 250),
...
train_names = "/data/shared/cmantill/training/ak15_Jul9/train/GravitonToHHToWWWW*/*1*.root"
test_names = "/data/shared/cmantill/training/ak15_Jul9/test/HHToBBVVToBBQQQQ_cHHH1/*1*.root"

branches = ["fj_genRes_mass",#"fj_genH_pt",                                                                                   
            #"label_4q",                                                                                                      
            "fj_H_WW_4q",
...
for events_dict in events:
    ...
    h_mass = events_dict.fj_genRes_mass[signal_mask][:1000]

    hists['fj_prop'].fill(
        msd = events_dict.fj_msoftdrop[signal_mask][:1000],
        pt = events_dict.fj_pt[signal_mask][:1000],
        pn = events_dict.fj_PN_H4qvsQCD[signal_mask][:1000],
        mass = events_dict.fj_mass[signal_mask][:1000],
        gmass = h_mass
    )
    ...
    '''
    hists['w_gen'].fill(
        wmass = events_dict.fj_genW_mass[signal_mask][:1000],
        wsmass = events_dict.fj_genWstar_mass[signal_mask][:1000],
        wpt = events_dict.fj_genW_pt[signal_mask][:1000],
        wspt = events_dict.fj_genWstar_pt[signal_mask][:1000],
        gmass = h_mass,
    )

    hists['fj_dr'].fill(
        gmass = h_mass,
        dr_W = events_dict.fj_dR_W[signal_mask][:1000],
        dr_Wstar = events_dict.fj_dR_Wstar[signal_mask][:1000],
        min_dr_Wdau = events_dict.fj_mindR_HWW_daus[signal_mask][:1000],
        max_dr_Wdau= events_dict.fj_maxdR_HWW_daus[signal_mask][:1000],
    )

masses = [125,120]
masses += list(range(50,250,20))
masses.remove(70)
masses.remove(110)
masses.remove(150)

invmasses = list(reversed(range(50,250,20)))
invmasses += [120,125]
invmasses.remove(70)
    ...
    for var in variables:
        for i in range(2):
            ...
            if var in ['dr_W','dr_Wstar','min_dr_Wdau','max_dr_Wdau','msd','mass']:
                lmasses = list(range(50,250,20)) + [120,125]
                lmasses.remove(70)
                lmasses.remove(110)
                lmasses.remove(150)
